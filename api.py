...
def configure_ngrok():
    ngrok_token = "2pR1Ulq0bAUxQzw9BDRbZ8DVdOQ_2BsWR45aweuuWc1FYFBi2"
    try:
...
def update_soul_txt(public_url):
    with open("anony8.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in anony8.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony8.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony8.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony8.txt on VPS: {str(e)}")
